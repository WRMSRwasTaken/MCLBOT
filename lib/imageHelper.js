...
class ImageHelper {
  ...
  async downloadImage(url, fapiEndpoint = false, fapiPostData = undefined) {
    if (!fapiEndpoint && !await this.main.stringUtils.checkUrl(url)) {
    ...
    return new Bluebird(async (resolve, reject) => {
      ...
      } catch (err) { // This is still going to be executed if for example a timeout happens, TODO: add nicer error messages, for example ENOTFOUND = server not found
