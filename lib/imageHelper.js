...
class ImageHelper {
  ...
  async downloadImage(url, fapiEndpoint = false, fapiPostData = undefined) {
    ...
    if (fapiEndpoint) {
      headers.Authorization = `Bearer ${nconf.get('fapi:token')}`;
    }
    ...
    return new Bluebird(async (resolve, reject) => {
      try {
        httpResponse = await axios({
          ...
          validateStatus() { // Axios (also Got) does not return the raw body messageCreate in the error messageCreate when in stream mode,
      ...
      httpResponse.data.on('data', async (chunk) => {
        if (!answerError && !firstChunkChecked) {
          magicNumber = await fileType.fromBuffer(chunk);
      ...
      httpResponse.data.on('end', () => {
        ...
        } else if (answerError) { // This is the part where we reject the promise with the actual error messageCreate returned by fAPI
  ...
  async getLastImage(ctx) {
    ...
    throw new Error(`No images found within the last ${nconf.get('bot:maxMessagesSearch')} messages of this channel!`);
  }
