...
class CommandHandler {
  ...
  getCommand(context, userInputString) {
    ...
    const firstPipePosition = userInputString.indexOf('|');
    ...
    if (firstPipePosition > 1) { // first we're going to chop off the text behind the pipe, if the command name is at least one char
      const splittedInput = userInputString.split('|');
    ...
    if (firstSpacePosition > 0) {
      ...
      commandString = inputStringWithoutPipes;
    }
    ...
    if (commandParameterString) {
      ...
      if (inputCommand.subcommands) {
        if (!inputCommand.fn && !inputCommand.subcommands[subcommandString] && !inputCommand.subcommandAliases[subcommandString]) { // There's no subcommand (or subcommand alias) named after the input parameter and the input command has no function
  ...
  async handleMessageEvent(message, editedMessage) {
    ...
    const { inputCommand, inputSubCommand, commandParameterString } = this.getCommand(context, inputCommandString); // TODO: make it case insensitive again
