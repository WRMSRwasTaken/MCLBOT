...
class CommandHandler {
  ...
  async initializeContext(message) {
    ...
    message.messageEdits = (message.isEdited) ? message.messageEdits + 1 : 0; // we have to build this for ourselves
    ...
    context.isEdited = !!message.isEdited;

    context.messageEdits = message.messageEdits;
    ...
    if (!context.isDM) {
      ...
      const mentionExec = this.main.mentionRegex.exec(message.content);
    ...
    if (!context.isDM || (nconf.get('bot:selfbot') && nconf.get('bot:selfbot') !== 'false')) {
      if (context.guildPrefixDisabled) {
        ...
        context.startsWithPrefix = message.content.startsWith(context.guildPrefix);
      }
    }
    ...
    context.deleteReplies = async () => {
      const tempReplies = message.replies.slice(); // we need to copy the array to delete old messages, but don't delete new (already added) replies
  ...
  async shouldHandle(context, message) {
    if (!context.isDM && context.isEdited && message.replies.length === 0) { // message edit in guild channel without answers, but handled before
    ...
    winston.debug('Message edit event fired! id: %s - old content: %s - new content: %s', message.id, message.content);
  ...
  async hasPermission(context) { // TODO: add permission flags for subcommands too (and maybe other things too)
    ...
    if ((this.getProperty(context, 'guildOnly') || permission || noConcurrent === 'guild') && context.isDM) { // no DM check
      context.reply('Sorry, but this command can\'t be executed via DM.');
      return false;
    }

    if (!context.isDM && permission && !context.member.hasPermission(permission) && !context.isBotAdmin) { // guild permission check for the calling member
    ...
    if (!context.isDM && selfPermission && !context.channel.can(selfPermission)) { // guild permission check for the bot member itself
  ...
  getCommand(context) {
    ...
    if (!inputCommand) {
      ...
      } else if (context.channel.can('ADD_REACTIONS')) {
  ...
  async messageSendFunction(context, args) {
    ...
    try {
      if (context.guild && !context.channel.can('SEND_MESSAGES')) {
        if (!context.channelNoPermission) { // Just send this message only 1 time per invoked command
          const permErrMsg = await context.author.createMessage(`Sorry, but I don't have the permission to send messages in the channel <#${context.channel.id}> on server ${context.guild.name}! Your command output is going to be my next DM.`);
      ...
      if (args.length !== 0 && typeof args[0] === 'string' && args[0].length > 2000) {
        if (context.guild && !context.channel.can('ATTACH_FILES')) {
          newMessage = await sendTo.createMessage('Sorry, but I don\'t have the permission to attach messages in this channel.');
        } else {
          newMessage = await sendTo.createMessage({
            files: [{
        ...
        newMessage = await sendTo.createMessage(...args);
      }
  ...
  async handleMessageEvent(message, isMessageEdit) {
    if (nconf.get('bot:selfbot') && nconf.get('bot:selfbot') !== 'false' && message.author.id !== this.main.api.user.id) {
      return false;
    }
    ...
    const context = await this.initializeContext(message);

    if (!await this.shouldHandle(context, message, isMessageEdit)) {
      return false;
    }
