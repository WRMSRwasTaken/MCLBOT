...
class CommandHandler {
  ...
  async handleMessageDeleteEvent(message) {
    ...
    winston.debug('Message delete event fired! id: %s - was answered by me: %s', message.id, !!(context.replies && context.replies.length));
    if (!context.replies || !context.replies.length) {
      return;
    }
