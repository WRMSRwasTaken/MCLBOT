...
class CommandHandler {
  ...
  async shouldHandle(context, message, editedMessage) {
    ...
    if (context.isDM && !messageToHandle.content.startsWith(context.serverPrefix) && nconf.get('bot:selfbot')) {
      return false;
    }
      ...
      for (let argIndex = 0; argIndex < commandToHandle.arguments.length; argIndex++) {
        ...
        if (inputCommandParams.length === 0) {
          if (!commandArgument.optional) {
            return context.reply(this.main.stringUtils.argumentsError(inputCommand, inputSubCommand, argIndex, 'Missing argument'));
          }

          if (commandArgument.default) {
            paramsToPass.push(commandArgument.default(context));
          } else if (commandArgument.default) {
        ...
        winston.debug(`Trying to parse argument parameter '${currentArgument}' for command ${inputCommand.name}${(inputSubCommand) ? ` ${inputSubCommand.name}` : ''} as type ${argumentParser.name}`);

        try {
          const parsed = await this.main.types[commandArgument.type].parse(currentArgument, commandArgument, context);

          paramsToPass.push(parsed);
        } catch (ex) {
          return context.reply(this.main.stringUtils.argumentsError(inputCommand, inputSubCommand, argIndex, ex.message));
        }
      }
    }
