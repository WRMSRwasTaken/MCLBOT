...
class CommandHandler {
  ...
  async sendFunction(message, args, editMessage = false) {
  ...
  async handleMessage(message, editMessage) {
    ...
    message.send = async (...args) => {
      if (editMessage && typeof args[0] === 'string' && args[0] === editMessage.content) { // We must return the message object to allow further edits
        winston.debug('My edit would be the same message...');
        return editMessage;
      }

      return this.sendFunction(message, args, editMessage);
    };
    ...
    if ((this.main.commands[inputCommand].admin || this.main.commands[inputCommand].noDM) && this.main.utils.isDM(message)) {
