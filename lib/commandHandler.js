...
class CommandHandler {
  ...
  async startsWithPrefix(message) {
    ...
    return message.content.startsWith(serverPrefix);
  }

  async shouldHandle(message) {
    if (!this.isDM(message) && !this.isMention(message)) {
      return this.startsWithPrefix(message);
    }
  ...
  async handleMessageEvent(message, editedMessage) {
    ...
    if (message.author.bot || message.pinned) {
      return false;
    }

    if (editedMessage && (editedMessage.author.bot || editedMessage.pinned)) {
      return false;
    }

    if (editedMessage) { // a message was edited
      const mentionExec = this.main.mentionRegex.exec(editedMessage.content);
      mentionLength = (mentionExec && mentionExec[0].length) || 0;
      isMention = !!mentionLength;
      isDM = (editedMessage.channel.type === 'dm');

      if (!isMention && !isDM) {
        serverPrefix = await this.main.prefixHelper.getServerPrefix(editedMessage.guild.id);

        if (!editedMessage.content.startsWith(serverPrefix)) {
          if (message.responded) {
      ...
      message = editedMessage; // set the new message to be handled
      ...
      winston.debug('New message event fired. ID: %s -- %s', message.id, message.content);
    }
    ...
    console.log(isMention, mentionLength);
    ...
    if (this.isDM(message)) { // direct message channel
      ...
      const messageWithoutMention = message.content.substring(mentionLength).trim();
      ...
      const serverPrefixForMention = await this.main.prefixHelper.getServerPrefix(message.guild.id);

      if (!inputCommand) {
        return message.send(`Yes? <@${this.main.api.user.id}> help or \`${serverPrefixForMention}help\``);
      }

      if (inputCommand.startsWith(serverPrefixForMention)) {
        return message.send(`You don't need \`${serverPrefixForMention}\` in mentioned messages. Try without.`);
      }
      ...
      const messageWithoutPrefix = message.content.substring(serverPrefix.length).trim();
