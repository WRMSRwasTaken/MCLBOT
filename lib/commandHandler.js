...
class CommandHandler {
  ...
  async handleCommand(message, inputCommand, inputCommandParams) {
    if (this.isDM(message)) {
      winston.info(`${(this.main.api.shard) ? `Shard ${this.main.api.shard.id} -` : ''} ${inputCommand} by ${message.author.tag} (ID: ${message.author.id}) (DM)`);
    } else {
      winston.info(`${(this.main.api.shard) ? `Shard ${this.main.api.shard.id} -` : ''} ${inputCommand} by ${message.author.tag} (ID: ${message.author.id}) in channel #${message.channel.name} (ID: ${message.channel.id}) on server ${message.guild.name} (ID: ${message.guild.id})`);
    }
  ...
  async handleMessageEvent(message, editedMessage) {
    ...
    const isBotAdmin = this.main.owner.includes(message.author.id);

    messageToHandle.replies = messageToHandle.replies || [];

    messageToHandle._send = messageToHandle.send;
    messageToHandle.send = async (...args) => this.messageSendFunction(messageToHandle, args);
    ...
    if (isBotAdmin) {
      winston.debug(`User id ${messageToHandle.author.id} is bot admin, skipping all blacklist checks!`);
    } else {
      ...
      if (!this.isDM(messageToHandle)) {
        ...
        winston.debug(`Checking global blacklist status for server id ${messageToHandle.guild.id}`);

        const serverBlacklistResult = await this.main.blacklistHelper.getServerBlacklist(messageToHandle.guild.id);

        if (serverBlacklistResult) {
          winston.debug(`Server id ${messageToHandle.guild.id} has been globally blacklisted! Returning`);
          return false;
        }
        winston.debug(`Server id ${messageToHandle.guild.id} is not globally blacklisted`);
    ...
    if (isDM) { // direct message channel
      ...
      if (!inputCommand && this.hasAttachments(messageToHandle)) {
        return messageToHandle.send('Thanks for the picture :)');
      }

      if (messageToHandle.content.startsWith(this.main.prefixHelper.getDefaultPrefix())) {
        return messageToHandle.send(`You don't need the bot's default prefix \`${this.main.prefixHelper.getDefaultPrefix()}\` in private messages. Try without.`);
      }
      ...
      if (!inputCommand) {
        return messageToHandle.send(`Hi! How can I help you? For help, type <@${this.main.api.user.id}> help or \`${serverPrefix}help\``);
      }

      if (inputCommand.startsWith(serverPrefix)) {
        return messageToHandle.send(`You don't need the bot's server prefix \`${serverPrefix}\` in mentioned messages. Try without.`);
      }

      if (inputCommand.startsWith(this.main.prefixHelper.getDefaultPrefix())) {
        return messageToHandle.send(`You don't need the bot's default prefix \`${this.main.prefixHelper.getDefaultPrefix()}\` in mentioned messages. Try without.`);
      }
    ...
    if (!this.main.commands[inputCommand] && !this.main.aliases[inputCommand]) {
      ...
      const related = this.main.stringUtils.findRelated(inputCommand);
    ...
    if (this.main.commands[inputCommand].owner && !isBotAdmin) {
    ...
    if (this.main.commands[inputCommand].admin && !this.main.stringUtils.isGuildAdmin(messageToHandle)) {
    ...
    if (this.main.commands[inputCommand].args
      ...
      return messageToHandle.send(this.main.stringUtils.argumentsError(inputCommand, inputCommandParams.length, 'Arguments missing'));
    }

    const redisResult = await this.main.redis.get(`cooldown:${messageToHandle.author.id}:${inputCommand}`);

    if (!isBotAdmin && redisResult) {
      if (redisResult === '2' && !editedMessage) {
        return false;
      }
    ...
    return this.handleCommand(messageToHandle, inputCommand, inputCommandParams);
  }
}
