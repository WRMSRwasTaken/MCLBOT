...
class CommandHandler {
  ...
  getCommand(context, inputCommandString, inputCommandParamString) {
    ...
    for (const char of inputCommandParamString.trim().split('')) {
      if (char === '"') {
  ...
  async executeCommand(context, command, commandParameters) {
    ...
    try {
      const output = await command.fn(context, commandParameters);
  ...
  async handleMessageEvent(message, editedMessage) {
    ...
    if ((context.isDM || !['295302159530983425'].includes(context.channel.id)) && !context.isBotAdmin) {
      context.reply('Sorry, but I don\'t handle command messages at the moment. Please check again later.');
      return false;
    }

    // This part here is utterly broken as I cba to work on it and fix it
    ...
    return this.executeCommand(context, inputSubCommand || inputCommand, inputCommandParams);
  }
}
