...
class Utils {
  ...
  isBotAdmin(message) {
    return this.main.owner.includes(message.author.id);
  }
  ...
  listCommandsString() {
    ...
    _.forEach(this.main.commands, (value, key) => {
      if (value.hide) {
        return;
      }
  ...
  displayHelpPage(pageNumber) {
    ...
    let output = `Help page: ${pageNumber} / ${this.main.helpPages.length}`;
  ...
  argumentsError(command, argIndex, message) {
    let hyphenCount;
    ...
    let sendMsg = `${message}:\n\`\`\``;
    sendMsg += `${command} ${this.listArgsString(command)}${(command.args && command.args[0]) ? ' ' : ''}${this.listOptionalArgsString(command)}\n`;
