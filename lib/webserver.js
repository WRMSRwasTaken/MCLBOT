...
class Webserver {
  ...
  init() {
    ...
    if (listenIsUnixSocket) {
      winston.debug('Setting permissions for webserver listen unix socket to: %d', nconf.get('webserver:listenUmask'));
    ...
    winston.info('Webserver listening on: %s', (listenIsUnixSocket) ? 'unix socket:' : 'port:', nconf.get('webserver:listen'));
  }
