...
class StringUtils {
  ...
  async checkUrl(url) {
    ...
    winston.debug('Going to check, if "%s" is a valid url', url);
  ...
  async getEmojiUrl(input) {
    ...
    if (!await this.main.stringUtils.checkUrl(url)) {
      return false;
    }
  ...
  formatUnixTimestamp(timestamp) {
    ...
    if (timestamp < Date.now()) {
      appendix = `${prettyMs(Date.now() - timestamp, { verbose: true })} ago`;
    } else {
      appendix = `in ${prettyMs(timestamp - Date.now(), { verbose: true })}`;
    }
