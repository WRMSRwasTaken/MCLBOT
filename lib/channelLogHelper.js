...
class ChannelLogHelper {
  ...
  async sendLogMessage(type, messageData) {
    ...
    if (this.main.api.shard) {
      messageData.shardID = this.main.api.shard.id;
    }
    ...
    if (nconf.get('bot:logchannel')) {
      ...
      if (this.main.api.shard) {
        ...
        this.main.api.shard.send({
          ...
          messageData: {
            shardID: this.main.api.shard.id,
          },
        });
      }
    }
  ...
  sendReadyMessage(data) {
    this.main.api.channels.get(nconf.get('bot:logchannel')).send(`Shard **${data.shardID}**'s websocket received READY event after ${prettyMs(data.readyDuration)}`);
  }

  sendDisconnectMessage(data) {
    this.main.api.channels.get(nconf.get('bot:logchannel')).send(`Shard **${data.shardID}**'s websocket disconnected with code ${data.event.code}`);
  }

  sendReconnectingMessage(data) {
    this.main.api.channels.get(nconf.get('bot:logchannel')).send(`Shard **${data.shardID}**'s websocket is reconnecting`);
  }

  sendResumedMessage(data) {
    this.main.api.channels.get(nconf.get('bot:logchannel')).send(`Shard **${data.shardID}** resumed the connection after ${prettyMs(data.reconnectDuration)}`);
  }
}
