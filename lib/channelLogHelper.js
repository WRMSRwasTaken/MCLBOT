...
class ChannelLogHelper {
  ...
  sendResumedMessage(data) {
    this.main.api.channels.get(nconf.get('bot:logchannel')).send(`${(data.shardID) ? `Shard **${data.shardID}**'s w` : 'W'}ebsocket received RESUMED event after ${prettyMs(data.reconnectDuration)} - replayed ${prettyMs(data.replayed)} events`);
  }
}
