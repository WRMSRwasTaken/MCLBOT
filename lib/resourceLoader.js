...
class ResourceLoader {
  ...
  async loadCommand(commandName, commandCategory, reload = false) {
    ...
    winston.debug(`Command ${(reload) ? 're' : ''}loaded:`, newCommand.name);
  }
  ...
  loadMiddleware(middlewareName) {
    ...
    try {
      ...
      winston.error('Error loading middleware file:', ex.message);
      throw new Error('Error loading middleware file!');
    }

    if (!Object.keys(newMiddleware).length) {
      winston.warn('Skipping empty middleware file:', middlewarePath);
      return;
    }
    ...
    winston.debug('Middleware loaded:', newMiddleware.name);
  }
