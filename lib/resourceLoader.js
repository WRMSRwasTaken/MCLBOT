...
class ResourceLoader {
  constructor(main) {
    ...
    this.main.permissions = {
      ...
      MANAGE_NICKNAMES: 'Manage other members\' nicknames',
    ...
    _.forEach(this.main.commands, (command) => {
      ...
      if (command.alias) {
        if (Array.isArray(command.alias)) {
          output += ` aliases are: ${command.alias.join(', ')}`;
        } else {
          output += ` aliases are: ${command.alias}`;
        }
      }
  ...
  validateCommandObject(commandObject, rootCommand) {
    Object.keys(commandObject).forEach((commandObjectKey) => {
    ...
    if (commandObject.alias && Array.isArray(commandObject.alias)) {
      commandObject.alias.forEach((alias) => {
    ...
    if (commandObject.permission) {
      if (Array.isArray(commandObject.permission)) {
        commandObject.permission.forEach((permission) => {
    ...
    if (commandObject.selfPermission) {
      if (Array.isArray(commandObject.selfPermission)) {
        commandObject.selfPermission.forEach((selfPermission) => {
    ...
    if (commandObject.arguments) {
      ...
      commandObject.arguments.forEach((argument) => {
    ...
    if (commandObject.flags) {
      ...
      Object.keys(commandObject.flags).forEach((flagName) => {
        ...
        if (flag.short) {
          ...
          if (rootCommand && rootCommand.shortFlags && rootCommand.shortFlags[flag.short] && rootCommand.shortFlags[flag.short].global) {
            winston.warn(`Short flag: ${flag.short} has been already defined in the root command as global flag, skipping short alias.`);
            return;
          }
          ...
          commandObject.shortFlags[flag.short] = flagName;
        }
      });
    }
  }
  ...
  async loadCommand(commandName, commandCategory, reload = false) {
      ...
      _.forEach(newCommand.subcommands, (subcommand, subcommandName) => {
        ...
        if (subcommand.alias) {
          if (!Array.isArray(subcommand.alias)) {
            ...
            _.forEach(subcommand.alias, (newAlias) => {
    ...
    winston.debug('Command loaded:', newCommand.name);
  }
  ...
  loadEvent(eventName) {
    ...
    winston.debug('Event loaded:', eventName);
  }
  ...
  executeTask(task, main) {
    ...
    // winston.debug('Executing task:', task.name);
  ...
  loadTask(taskName) {
    ...
    if (newTask.noSelfbot && nconf.get('bot:selfbot')) {
      winston.debug('Skipped loading task in selfbot mode:', taskName);
    ...
    winston.debug('Task loaded:', taskName);
  }

  startTasks() {
    Object.keys(this.main.tasks).forEach((taskName) => {
      winston.debug('Starting task:', taskName);
  ...
  loadType(typeName) {
    ...
    winston.debug('Command parameter type loaded:', newType.name);
  }
