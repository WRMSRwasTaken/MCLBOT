...
class ResourceLoader {
  ...
  loadEvent(eventName, displayName) {
    ...
    if (newEvent.debounce && typeof newEvent.debouncedFn !== 'function') {
      throw new Error('The "debouncedFn" property of an imported object MUST be a function if "debounce" is set to true');
    }
    ...
    this.main.api.on(displayName || eventName, async (...args) => {
      if (!newEvent.debounce) {
        newEvent.fn.apply(null, [].concat(this.main, args));

        return;
      }

      this.main.debouncedEvents[displayName || eventName] = newEvent;

      const data = await newEvent.fn.apply(null, [].concat(this.main, args));

      if (!data) {
        return;
      }

      this.main.eventDebouncer.addEvent(displayName || eventName, data.key, data.payload);
    });

    winston.debug(`${(newEvent.debounce) ? 'Debounced event' : 'Event'} loaded: ${(displayName) ? `${displayName} -> ${eventName}` : eventName}`);
  }
