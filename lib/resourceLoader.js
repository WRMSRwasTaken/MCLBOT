...
class ResourceLoader {
  ...
  loadCommand(commandName, commandCategory, reload) {
    ...
    if (newCommand.arguments && typeof newCommand.arguments !== 'object') {
      winston.error('Not adding command: %s, the "arguments" property of an imported object MUST be an array!', newCommand.name);
      return;
    }

    if (newCommand.arguments) {
      _.forEach(newCommand.arguments, (argument) => {
  ...
  loadType(typeName) {
    ...
    //
    // if (!newType.validate || typeof newType.validate !== 'function') {
    //   throw new Error('The "validate" property of an imported object MUST be a function');
    // }
