...
class ResourceLoader {
  ...
  validateCommandObject(commandObject, isSubCommand = false) {
    ...
    if (isSubCommand) {
      ...
      if (commandObject.fn && typeof commandObject.fn !== 'function') {
        throw new Error('The "fn" property of a command must be none or a function');
      }

      if (commandObject.fn && (!commandObject.description || typeof commandObject.description !== 'string')) {
        throw new Error('The "description" property of a command must be none or, if "fn" has been set, a string');
      }
    }
  ...
  async loadCommand(commandName, commandCategory, reload = false) {
    ...
    this.validateCommandObject(newCommand, false);
    ...
    if (newCommand.subcommands) {
      ...
      _.forEach(newCommand.subcommands, (subcommand, subcommandName) => {
        ...
        this.validateCommandObject(subcommand, true);
  ...
  loadTask(taskName) {
    ...
    this.main.tasks[taskName] = setInterval(this.executeTask.bind(null, newTask, this.main), newTask.interval * 1000);
