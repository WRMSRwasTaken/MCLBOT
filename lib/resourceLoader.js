...
class ResourceLoader {
  constructor(main) {
    ...
    this.main.permissions = {
      ...
      READ_MESSAGE_HISTORY: 'Read messageCreate history',
  ...
  async initializeModule() {
    winston.debug('Resource loader is going to load bot resources...');
    await this.loadTypeFiles();
  ...
  async reloadCategory(category) {
    ...
    for (const command of this.main.categories[category]) {
      await this.loadCommand(command, null, true);
    }
  }

  async reloadAllCommands() {
    for (const command of Object.keys(this.main.commands)) {
      await this.loadCommand(command, null, true);
    }
  }
  ...
  validateCommandObject(commandObject, rootCommand) {
    ...
    if (rootCommand) { // subcommand
      ...
      if (commandObject.load && typeof commandObject.load !== 'function') {
        throw new Error('The "load" property of a command must be none or a function');
      }
    }
  ...
  async loadCommand(commandName, commandCategory, reload = false) {
    ...
    if (newCommand.load) {
      winston.debug('Command %s has "load" property, executing function...', newCommand.name);

      await newCommand.load(this.main);
    }
  ...
  loadEvent(eventName, displayName) {
    ...
    this.main.api.on(displayName || eventName, async (...args) => {
      newEvent.fn.apply(null, [].concat(this.main, args));
    });

    winston.debug(`Event loaded: ${(displayName) ? `${displayName} -> ${eventName}` : eventName}`);
  }

  async executeTask(task, main) {
    ...
    try {
      ...
      winston.error('Error executing task %s: %s', task.name, err.message);
    }
  }
  ...
  async loadTask(taskName) {
    ...
    if (this.main.api.shard && this.main.api.shard.ids[0] !== 0 && newTask.shardZeroOnly) {
      winston.debug('Disabling task %s as we are not shard zero.', taskName);
      return;
    }
    ...
    if (newTask.load) {
      winston.debug('Task %s has "load" property, executing function...', newTask.name);

      await newTask.load(this.main);
    }
  ...
  startTasks() {
    for (const taskName of Object.keys(this.main.tasks)) {
      ...
      this.main.runningTasks[taskName] = setInterval(this.executeTask.bind(null, this.main.tasks[taskName], this.main), this.main.tasks[taskName].interval * 1000);
    }
  }

  loadTaskFiles() { // TODO: make this async
    fs
