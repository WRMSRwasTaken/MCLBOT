...
class ResourceLoader {
  ...
  loadCategory(category) {
    ...
    if (category) {
      try {
        const filePath = path.resolve('./commands', `${category}/category.json`); // `${dirName}/category.json`;

        fs.accessSync(filePath);

        winston.debug('Category %s has a category override file!', category);
        ...
        try {
          const override = require(filePath);

          console.log(category)
          console.log(override)

          this.main.categoryOverrides[category] = override;
        } catch (ex) {
          winston.error('Error loading override file:', ex.message);
          throw new Error(`Error loading override file:\n\n${ex.message}`);
        }
      } catch (ex) {
        winston.error(ex);
      }
  ...
  validateCommand(commandObject, isSubCommand = false) {
    if (commandObject.alias && !Array.isArray(commandObject.alias) && typeof commandObject.alias !== 'string') {
      throw new Error('The "alias" property of an imported object MUST be none, an array or a string!');
    }
  ...
  addAlias(command, alias) {
    if (typeof alias !== 'string') {
      throw new Error('The "alias" property of an imported object MUST be a string or an array of strings!');
    }

    if (this.main.commands[alias]) {
      winston.warn('Not adding alias command: %s for command: %s, command already exists!', alias, command);
      return;
    }

    if (this.main.aliases[alias]) {
      winston.warn('Not adding alias command: %s for command: %s, alias already exists! (used by command %s)', alias, command, this.main.aliases[alias]);
      return;
    }

    this.main.aliases[alias] = command;
    winston.debug('Added alias: %s -> %s', alias, command);
  }
