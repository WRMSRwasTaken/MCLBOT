...
class PaginationHelper {
  ...
  async handleMessageReactionAddEvent(messageReaction, user) {
    winston.debug('raw message reaction add event fired. Added emoji %s', messageReaction.emoji);
    winston.debug('has pagination?', !!messageReaction.message.pagination);
    winston.debug('imvoker user id', (messageReaction.message.pagination) ? messageReaction.message.pagination.invokerUserID : '-');
    winston.debug('react user id', user.id);
    winston.debug('same user?', (messageReaction.message.pagination) ? (messageReaction.message.pagination.invokerUserID !== user.id) : '-');
