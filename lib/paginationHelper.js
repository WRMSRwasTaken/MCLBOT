...
class PaginationHelper {
  ...
  initPagination(context, pageCount, message = context.replies[0]) {
    if (context.guild && pageCount > 1 && !context.channel.permissionsFor(context.guild.me).has('ADD_REACTIONS')) {
    ...
    winston.debug('Initializing pagination on message ID: %s with %d pages', message.id, pageCount);
  ...
  async createPaginatedEmbedList(context, title, pageCount) {
    const message = await context.reply('Loading list, please wait...');

    const pagination = await this.initPagination(context, pageCount, message);
    ...
    pagination.on('paginate', (pageNumber) => {
      currentPage = pageNumber;
    });

    pagination.on('update', (content, newPageCount = false, entryCount = false) => {
      ...
      if (pageCount > 1) {
        embed.setFooter(`Page ${currentPage}/${(newPageCount) ? `${newPageCount}` : `${pageCount}`} ${(entryCount) ? `(${entryCount} entries)` : ''}`);
      } else if (entryCount) {
