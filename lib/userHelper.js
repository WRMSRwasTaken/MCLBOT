...
class UserHelper {
  constructor(main) {
    ...
    this.mentionRegex = XRegExp('^(<@!?)?(?<userID> \\d{16,})>?$', 'isx');
  }
  ...
  async getUser(message, argString) {
    ...
    if (message.guild) {
      const guildMember = this.getGuildMember(message, argString); // This request has been sent in a guild channel, so let's grab the user object from there
