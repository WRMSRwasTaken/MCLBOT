...
class UserHelper {
  ...
  async getGuildMember(context, input) {
    ...
    memberMatch = memberMatch.sort((memberOne, memberTwo) => {
      if (memberOne.presence.status !== 'offline' && memberTwo.presence.status === 'offline') { // get the member being online right now, if both have no sent message timestamp record
        return -1;
      } else if (memberTwo.presence.status !== 'offline' && memberOne.presence.status === 'offline') {
        return 1;
      }
  ...
  checkGuildMemberHierarchy(context, target) {
    if (context.isBotAdmin || context.guild.ownerID === context.author.id) {
      return true;
    }

    return context.member.roles.highest.position > target.roles.highest.position;
  }
}
