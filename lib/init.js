...
class Init {
  ...
  initBase() {
    this.main.shutdown = async (code) => {
      ...
      setTimeout(() => {
        winston.warn('Shutdown timeout after 20 seconds. Going to exit the hard way.');
  ...
  async connectToDAPI() {
    ...
    this.main.api.on('shardDisconnected', (event) => {
      if (!this.main.ready) {
        return;
      }
    ...
    this.main.api.on('shardReconnecting', () => {
      if (!this.main.ready) {
        return;
      }
    ...
    this.main.api.on('shardResumed', (replayed) => {
