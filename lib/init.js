...
class Init {
  ...
  initAPI() {
    ...
    if (!this.main.api.shard) {
      ...
      winston.info(`         --- core: ${(this.main.version) ? this.main.version : 'N/A'} - api: ${this.main.Discord.version} ---`);
      winston.info('');
  ...
  checkSettings() {
    ...
    if (!nconf.get('bot:prefix')) {
      winston.error('No bot prefix has been specified, refusing to start without any default prefix set! Exiting.');
      process.exit(1);
    }
  ...
  initBase() {
    ...
    if (this.main.devMode && !this.main.api.shard) {
      winston.info('Development mode activated.');
    }
  }
  ...
  async initSQL() {
    ...
    Object.keys(this.main.db).forEach((modelName) => {
  ...
  connectToDAPI() {
    this.main.api.on('ready', async () => {
      winston.info(`${(this.main.api.shard) ? `Shard ${this.main.api.shard.id} ready` : 'Ready'} event has been fired! Registering event handlers...`);
    ...
    this.main.api.on('resumed', (replayed) => {
      this.main.ready = true;
      winston.info(`${(this.main.api.shard) ? `Shard ${this.main.api.shard.id} resumed` : 'Resumed'} connection to Discord API after ${prettyMs(Date.now() - this.main.disconnectTime)}. Replayed ${replayed} events.`);
    });
