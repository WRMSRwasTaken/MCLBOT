...
class Init {
  ...
  connectToDAPI() {
    ...
    this.main.api.on('resumed', (replayed) => {
      this.main.ready = true;
      winston.info(`${(this.main.api.shard) ? `Shard ${this.main.api.shard.id} resumed` : 'Resumed'} connection to Discord API after ${prettyMs(Date.now() - this.main.disconnectTime)}. Replayed ${replayed} event(s).`);
    });
