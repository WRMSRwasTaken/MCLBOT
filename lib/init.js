...
class Init {
  ...
  connectToDAPI() {
    this.main.api.on('ready', async () => {
      ...
      winston.info(`Ready event has been fired after ${(Date.now() - this.main.startedTime) / 1000} second(s). Registering event handlers and starting tasks...`);
      ...
      if (!this.main.eventsRegistered) { // My bot sometimes starts answering multiple times, but I can't tell why. Could be a bug in discord.js v12. I'm adding this simple check hoping this will help
        ...
        winston.warn('A second ready event has been fired! Skipping registering event handlers and starting tasks to avoid double answering messages.');
      }
    ...
    this.main.api.on('disconnect', (event) => {
      this.main.ready = false;
      winston.warn('Disconnected from Discord API! Code: %d', event.code); // TODO: pause tasks on connection loss
    });
    ...
    this.main.api.on('reconnecting', () => {
      ...
      winston.warn(`Lost connection to Discord API${(this.main.api.ws.connection.lastPingTimestamp) ? ` (Last heartbeat was ${prettyMs(Date.now() - this.main.api.ws.connection.lastPingTimestamp)} ago)` : ''}! Reconnecting...`);
    });
