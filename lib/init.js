...
class Init {
  ...
  initLog() {
    ...
    process.on('uncaughtException', (err) => {
      ...
      winston.error(`${(this.main.api.shard) ? `Shard ${this.main.api.shard.id} uncaught` : 'Uncaught'} exception: %s`, err.message);
    });
    ...
    process.on('unhandledRejection', (reason) => {
      ...
      winston.error(`${(this.main.api.shard) ? `Shard ${this.main.api.shard.id} uncaught` : 'Uncaught'} promise Error: %s`, reason.message);
    });
  ...
  createRPCSocket() {
    net.createServer((socket) => {
      winston.debug('RPC socket connection from: %s', socket.remoteAddress);
      ...
      socket.on('data', async (data) => {
        try {
          ...
          winston.error('RPC socket error: %s', ex.message);
        }
      });

      socket.on('end', () => {
        winston.debug('RPC socket disconnect from: %s', socket.remoteAddress);
      });
    }).listen(nconf.get('rpc:port'));
    winston.info('RPC socket listening on port: %s', nconf.get('rpc:port'));
  }
  ...
  connectToDAPI() {
    ...
    this.main.api.on('disconnect', (event) => {
      this.main.ready = false;
      winston.warn(`${(this.main.api.shard) ? `Shard ${this.main.api.shard.id} disconnected` : 'Disconnected'} from Discord API! Code: %d`, event.code);
    });
    ...
    this.main.api.login(nconf.get('bot:token'))
      .catch((err) => {
        winston.error('Unable to connect to Discord API! %s', err.message);
        this.main.shutdown(1);
      });
  }
