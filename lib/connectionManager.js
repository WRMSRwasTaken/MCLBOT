...
class ConnectionManager {
  ...
  async onReady() {
    ...
    if (!this.firstReady) {
      ...
      winston.info(`Ready event has been fired after ${prettyMs(Date.now() - this.preConnectTime)}.`);
  ...
  async onShardResumed(replayed) {
    if (!this.firstReady) {
      winston.error('Resumed event has been fired although there was no ready event before! Exiting...');
